...
// Se hai già un tipo/config auth tipizzata (es. TypedConfigService o AuthConfig),
// puoi usarla al posto di ConfigService. Workshop-wise va benissimo ConfigService.

@Module({
  imports: [
    UsersModule,

    // Repository disponibili in questo modulo (necessario anche al gateway per Game)
    TypeOrmModule.forFeature([Game, GameParticipant]),

    // JwtService disponibile nello stesso contesto del gateway
    // Usiamo registerAsync così prendiamo secret/exp dall'env tramite ConfigService
    ConfigModule, // serve se non è già globale; se è globale, non fa male
    JwtModule.registerAsync({
      imports: [ConfigModule],
      inject: [ConfigService],
      useFactory: (configService: ConfigService) => {
        // Allinea questi nomi alle tue env / config.
        // Dal tuo CLAUDE.md: JWT_SECRET, JWT_EXPIRES_IN
        const secret = configService.get<string>("JWT_SECRET");
        const expiresIn = configService.get<string>("JWT_EXPIRES_IN");

        if (!secret) {
          // Fail fast: meglio scoprire subito in workshop
        ...
        return {
          secret,
          signOptions: {
            // expiresIn è opzionale per verify, ma lasciamo coerente la config JWT
  ...
  exports: [
    // Di solito non serve esportare GameGateway a meno che altri moduli lo iniettino.
    // Lo lasciamo vuoto per ora.
  ],
})
